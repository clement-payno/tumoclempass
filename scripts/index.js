...
function generatePassword(length) {
   ...
   for(let i = 1; i <= length; i++ ){
      if(shouldIncludeSymbols()){
       pourPass.push(randomSymbol())
       }

      if(shouldIncludeNumbers()){
        pourPass.push(randomNumber())
      }
    if(shouldIncludeLowerCase()){
      pourPass.push(randomLowerCase())
      }
      if(shouldIncludeUpperCase()){
      pourPass.push(randomUpperCase())
       }
 
    pass = pass + pourPass[randomInt(0,pourPass.length)]
    pourPass = []
